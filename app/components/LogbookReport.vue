...
// Printable SIWES logbook report for one student. Shared by the admin report
// page and the student's own "print my logbook" page. Access is enforced by
// RLS (the viewer only sees data they're allowed to read).
const props = defineProps<{ studentId: string; backTo: string; backLabel: string }>()
...
async function load() {
  ...
  const [p, a, e, s] = await Promise.all([
    ...
    client.from('assignments').select('*, supervisor:profiles!assignments_supervisor_id_fkey(full_name)').eq('student_id', id).maybeSingle(),
...
<template>
  <div class="min-h-screen bg-gray-100 print:bg-white">
    ...
    <div class="no-print sticky top-0 flex items-center justify-between border-b bg-white px-4 py-3 print:hidden">
      <NuxtLink :to="backTo" class="text-sm text-caleb-cyan-dark hover:underline">{{ backLabel }}</NuxtLink>
    ...
    <div v-else class="report mx-auto my-6 max-w-[800px] bg-white p-10 shadow print:my-0 print:max-w-none print:shadow-none">
      ...
      <section class="mt-10 break-inside-avoid">
        ...
        <div class="grid grid-cols-2 gap-8">
          ...
          <div>
            <div class="h-24 rounded border border-dashed border-gray-300"></div>
            <p class="mt-1 text-xs text-gray-500">Academic supervisor's signature &amp; date</p>
          </div>
        </div>
      </section>
